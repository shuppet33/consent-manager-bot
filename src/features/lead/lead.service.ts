...
async function getPostByStartParam(startParam: string) {
    return db.query(
        `SELECT * FROM posts WHERE start_param = $1`,
        [startParam]
    );
}

export async function leadFlow(conversation: Conversation, ctx: Context) {
    try {
        const startParam = ctx.match; // от deep link
        ...
        // 4. сохраняем
        await db.query(
            `INSERT INTO leads (user_id, phone, post_id, created_at)
             VALUES ($1, $2, $3, NOW())`,
            [
                ctx.from?.id,
                phone,
